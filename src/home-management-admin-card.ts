...
export class HomeManagementAdminCard extends LitElement {
  ...
  private _cancelEdit() { this._editing = undefined; this._editTask = ''; this._editDate = ''; this._editMode = 'date'; }
  private async _update() {
    ...
    try {
      if (this._editMode === 'daily' || this._editMode === 'weekdays') {
        ...
        this._items = this._items.map((entry) => entry.uid === item.uid ? { ...entry, summary: task, due: dueDate || entry.due } : entry);
