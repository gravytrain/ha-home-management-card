...
export class HomeManagementAdminCard extends LitElement {
  ...
  @state() private _moving?: string;
  @state() private _editing?: TodoItem;
  @state() private _editTask = '';
  @state() private _editDate = '';
  @state() private _editMode: ScheduleMode = 'date';
  @state() private _editDays = new Set<string>(['monday', 'tuesday', 'wednesday', 'thursday', 'friday']);
  ...
  private _dateKey(daysFromToday = 0) {
    const value = new Date();
    value.setDate(value.getDate() + daysFromToday);
    return String(value.getFullYear()) + '-' + String(value.getMonth() + 1).padStart(2, '0') + '-' + String(value.getDate()).padStart(2, '0');
  }
  private _isMissed(item: TodoItem) {
    const due = item.due?.slice(0, 10);
    return item.status !== 'completed' && !!due && due < this._dateKey();
  }
  private _currentItems() {
    const today = this._dateKey();
    return this._items.filter((item) => !item.due || item.due.slice(0, 10) >= today);
  }
  private _missedItems() { return this._items.filter((item) => this._isMissed(item)); }
  ...
  private _startEdit(item: TodoItem) {
    ...
    this._editMode = this._editDate === new Date().toISOString().slice(0, 10) ? 'today' : 'date';
    this._editDays = new Set(this._days);
    this._editTechTime = item.tech_time ?? false;
    this._message = '';
  }
  private _toggleEditDay(day: string) { const next = new Set(this._editDays); next.has(day) ? next.delete(day) : next.add(day); this._editDays = next; }
  private _cancelEdit() { this._editing = undefined; this._editTask = ''; this._editDate = ''; this._editMode = 'date'; this._editTechTime = false; }
  private async _update() {
    ...
    if (this._editMode === 'weekdays' && !this._editDays.size) { this._message = 'Choose at least one weekday.'; return; }
    this._updating = true; this._message = '';
    try {
      if (this._editMode === 'daily' || this._editMode === 'weekdays') {
        await this.hass.callService('home_management', 'schedule_item', { list_entity: entityId, item: task, recurrence: this._editMode, weekdays: this._editMode === 'weekdays' ? [...this._editDays] : [] });
        await this.hass.callService('todo', 'remove_item', { item: item.uid }, { entity_id: entityId });
        this._message = `Updated “${task}” to a repeating schedule.`;
      } else {
        const dueDate = this._editMode === 'today' ? new Date().toISOString().slice(0, 10) : this._editDate;
        const data: Record<string, unknown> = { item: item.uid, rename: task };
        if (dueDate) data.due_date = dueDate;
        data.description = JSON.stringify({ tech_time: this._editTechTime, order: item.order });
        await this.hass.callService('todo', 'update_item', data, { entity_id: entityId });
        this._items = this._items.map((entry) => entry.uid === item.uid ? { ...entry, summary: task, due: dueDate || entry.due, tech_time: this._editTechTime } : entry);
  ...
  private async _moveToTomorrow(item: TodoItem) {
    const entityId = this._entity();
    if (!this.hass || !entityId) return;
    const tomorrow = this._dateKey(1);
    const duplicate = this._items.some((entry) => entry.uid !== item.uid && entry.status !== 'completed' && entry.summary.trim().toLocaleLowerCase() === item.summary.trim().toLocaleLowerCase() && entry.due?.slice(0, 10) === tomorrow);
    if (duplicate) { this._message = '“' + item.summary + '” is already on tomorrow’s list.'; return; }
    this._moving = item.uid; this._message = '';
    try {
      await this.hass.callService('todo', 'update_item', { item: item.uid, due_date: tomorrow }, { entity_id: entityId });
      this._items = this._items.map((entry) => entry.uid === item.uid ? { ...entry, due: tomorrow } : entry);
      this._message = 'Moved “' + item.summary + '” to tomorrow.';
    } catch { this._message = 'Could not move this task. Check the To-do list supports due dates.'; }
    finally { this._moving = undefined; }
  }
  ...
  private async _save() {
    ...
    try {
      if (this._mode === 'today' || this._mode === 'date') {
        ...
        const description = JSON.stringify({ tech_time: this._techTime, order: this._items.length });
        await this.hass.callService('todo', 'add_item', { item: task, due_date: dueDate, description }, { entity_id: entityId });
      } else {
        await this.hass.callService('home_management', 'schedule_item', { list_entity: entityId, item: task, recurrence: this._mode, weekdays: this._mode === 'weekdays' ? [...this._days] : [], tech_time: this._techTime });
      }
      this._task = '';
      this._techTime = false;
  ...
  render() {
    const child = this._selected(); const entity = this._entity(); const currentItems = this._currentItems(); const missedItems = this._missedItems();
    return html`<ha-card><main><header><div><p class="eyebrow">FAMILY OPERATIONS</p><h1>${this._config.title}</h1><p class="subhead">Assign work without interrupting the children’s dashboard.</p></div><span class="badge">PARENT</span></header>
      ...
      <section class="submit-row"><div class="status ${this._message ? 'visible' : ''}">${this._message}</div><button class="submit" ?disabled=${this._saving || !entity} @click=${this._save}>${this._saving ? 'SAVING…' : `ADD TO ${child.name.toUpperCase()}'S ${this._kind.toUpperCase()}`}</button></section>
